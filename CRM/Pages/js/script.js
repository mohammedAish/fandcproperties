...
$(function() {
    ...
    if (!!scheds) {
        Object.keys(scheds).map(k => {
            var row = scheds[k]
            events.push({ id: row.id, title: row.title, start: row.start_datetime, end: row.end_datetime, status: row.status });
        });
    }
    ...
    calendar = new Calendar(document.getElementById('calendar'), {
        ...
        eventClick: function(info) {
            ...
            if (!!scheds[id]) {
                ...
                details.find('#status').text(scheds[id].status);
    ...
    $('#edit').click(function() {
        ...
        if (!!scheds[id]) {
            ...
            form.find('[name="status"]').val(scheds[id].status);
    ...
    $('#delete').click(function() {
        ...
        if (!!scheds[id]) {
            ...
            if (_conf === true) {
                location.href = "../Query/Calender/delete_schedule.php?id=" + id;
            }
        } else {
            alert("Event is undefined");
        }
